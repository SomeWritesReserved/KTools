	...
	public partial class CommandRunner
	{
		...
		public CommandRunner(IFileSystem fileSystem, System.IO.TextWriter outputWriter, System.IO.TextReader inputReader)
		{
			...
			this.commands = new Dictionary<string, Tuple<Action<Dictionary<string, object>>, string, string>>(StringComparer.OrdinalIgnoreCase)
			{
				...
				{ "catalog-create-detached-combined", new Tuple<Action<Dictionary<string, object>>, string, string>(this.commandCatalogCreateDetachedCombined, "<DirectoryOfCatalogs>",
					"Combines all existing detached catalogs in <DirectoryOfCatalogs> to create a new, combined catalog file. The catalog will be saved as a '.kcatalog' file in <DirectoryOfCatalogs> and have all of the files that are in all of the detached catalogs. This is intended for combining the existing detached catalogs of read-only media like CD/DVDs (see catalog-create-detached) in order to provide a way to know all of the files that have been previously backed up to read-only media.") },
